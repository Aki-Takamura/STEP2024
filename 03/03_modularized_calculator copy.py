...
def evaluate_paren(tokens):
    ...
    while index < len(tokens):#()内の計算
        ...
        if token['type'] in ['PAREN','ABS','INT','ROUND']:
...
def evaluate_multi_and_div(tokens):
    index = 1
...
def evaluate(tokens):
    ...
    tokens = evaluate_paren(tokens_copy)

    #優先順位2:掛け算，割り算の計算
    tokens_copy = tokens.copy()
    tokens = evaluate_multi_and_div(tokens_copy)

    #優先順位3:足し算，引き算の計算
    tokens_copy = tokens.copy()
    answer = evaluate_plus_and_minus(tokens_copy)
