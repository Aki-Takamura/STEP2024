    ...
    def delete(self, key):
        ...
        while item:
            if item.key == key:
                ...
                self.buckets[bucket_index] = None #<-これがダメなのはわかった
 
                print(self.buckets)               
                self.item_count -= 1 #一つ減らす
                return (True)
            prev = item
            item = item.next
        return (False)
...
def functional_test():
    ...
    assert hash_table.delete("acb") == True #リンクリスト内の処理ができていない
